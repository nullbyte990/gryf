...
-- Remap coastlines and handle contour attributes
function attribute_function(attr,layer)
	-- Handle contour elevation data
	if layer=="contour" then
		local ele = tonumber(attr["ele"])
		local cidx = tonumber(attr["cidx"])
		local level = tonumber(attr["level"])

		-- Keep contour attributes deterministic even if source columns are partially missing.
		if cidx == nil and ele ~= nil then
			if math.floor(ele) % 100 == 0 then cidx = 1 else cidx = 0 end
		end
		if level == nil then level = cidx end
